...
function Id(length) {
   ...
   for (let i = 0; i < length; i++) { 
      var comb = characters.charAt(Math.floor(Math.random() * characters.length));
...
setTimeout(() => {
   ...
   menu.style.left = "80%";
   menu.innerHTML = "<button id='run'>Run</button><br><br><input type='number' id='rate' min='8' max='30'></input><br><h4>Rate Of Speech (8 is min, 30 is max) NO SPAMME JOEL</h4><br><button id='stop'>Stop Spam</button>";
  document.getElementById("run").onclick = () => {
    var txt = protegentAds[Math.floor(Math.random() * protegentAds.length)];
    var ratedif = Math.floor(Math.random() * 501)
    var rate1 = document.getElementById("rate").value;
    var rate2 = parseInt(rate1);
    var rate = rate2 * 1000 + ratedif;
    setInterval(() => {
      if(stopall == false){
   ...
   document.getElementById("stop").onclick = () => {
       if(manualstop == false){
          ...
          document.getElementById("stop").innerHTML = "Continue Talking";
          return;
       } else {
         stopall = false;
         manualstop = false;
         console.log("manualstop:" + manualstop);
         document.getElementById("stop").innerHTML = "Stop Talking";
       }
    }
},600)
...
setInterval(() => {
   ...
   time = timed * 1000;
   var chanc = Math.floor(Math.random() * 20);
   if(chanc > 14){
   send("/color");}
